...
def test_claim_validator_unicode_claim():
    ...
    with patch("mcp_tools.claim_validator.retrieve_composite", return_value=[]):
        with patch("mcp_tools.claim_validator.retrieve_fulltext", return_value=[]):
            from mcp_tools.claim_validator import ClaimValidator
...
def test_retrieve_composite_maps_results_without_network():
    ...
    with patch.dict(
        ...
        results = retrieve_composite("test", max_sources=1)
...
def test_retrieve_fulltext_handles_error():
    """WhooshIndex wirft Exception → leere Liste."""
    ...
    with patch.dict(sys.modules, {"darknet_search.index": mock_whoosh}):
        results = retrieve_fulltext("test")
...
def test_retrieve_fulltext_maps_results_without_index():
    ...
    with patch.dict(
        ...
        results = retrieve_fulltext("test", max_sources=1)
